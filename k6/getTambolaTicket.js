...
const ids = JSON.parse(open(`subscribedTournaments.json`));
...
export default function () {
    ...
    let lifepackId = ids[playerId]["lifepackId"];

    // Checking if the lifepack ID is null.
    lifepackId = 0;

    const gameurl = 'http://localhost:9012/api/v1/game/tambola/ticket';
    ...
    data["lifepackId"] = lifepackId;
    data["ticket-id"] = jsonResponse["ticket-id"]; // Use bracket notation here
    ...
    writeString(`getTambolaTicket${playerId}.json`, getJsonified);
